        ...
        <div className="flex items-center gap-3 overflow-x-auto pb-2 pt-1 scrollbar-thin">
          {availableImages.map((url, idx) => {
            ...
            return (
              <div
                key={`${url}-${idx}`}
                className={`group relative shrink-0 aspect-square w-24 sm:w-28 overflow-hidden rounded-button border-2 transition-all cursor-pointer ${
                  isSelected
                    ? isPrimary
              ...
              >
                ...
                {isSelected ? (
                  <div
                    className={`absolute top-1 left-1 flex items-center justify-center rounded-badge px-1.5 py-0.5 text-[10px] font-bold text-white shadow-sm ${
                ...
                <div
                  className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-black/80 px-1.5 py-1 opacity-0 transition-opacity group-hover:opacity-100"
                ...
                >
                  {isSelected && !isPrimary ? (
                    <button
                      ...
                      className="text-[10px] font-semibold text-category-trans hover:underline"
                    >
                      Make primary
                    </button>
                  ) : (
