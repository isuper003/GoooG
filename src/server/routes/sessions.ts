...
import { dueIds, leechIds, criticalIds, quickMixIds } from '../queues';
import {
...
async function resolvePoolIds(
  ...
  if (body.preset) {
    ...
    switch (body.preset) {
      case 'due': {
        focusIds = await dueIds(db, nowIso);
        break;
      }
      case 'leech': {
        focusIds = await leechIds(db);
        break;
      }
      case 'critical': {
        focusIds = await criticalIds(db, nowIso);
        break;
      }
      case 'quick_mix': {
        focusIds = await quickMixIds(db);
        break;
      }
      default: {
